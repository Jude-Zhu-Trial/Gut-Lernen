...
// 每键的电压窗口 {min_mV, max_mV}。窗口间留 30mV 死区:配合去抖防止边界抖动
// 在相邻两键间反复横跳。OK 上界收窄到 900:松开态在 DB_12 下饱和读数 ≥2500mV,
// 仍有 ≥1600mV 裕量;原上界 1900 时噪声下探即产生幻影按键(自动翻卡)。
// 若实测某键读数不在窗口内,用 bsp_button_read_mv() 测出实际分压后按中点重定边界。
#define BSP_BTN_MV_TABLE  { {0, 140}, {170, 420}, {450, 900} }
